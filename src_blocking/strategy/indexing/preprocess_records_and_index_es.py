...
from src_blocking.preprocessing.entity_extraction import extract_entity
...
from src_blocking.preprocessing.language_detection import LanguageDetector
from src_blocking.strategy.retrieval.retrieval_strategy import load_es_index_configuration
...
def load_data(dataset, worker, tokenizer, no_test, with_language_detection, duplicate_check, entity_length_check, switched):
    ...
    # Connect to Elasticsearch (ES 8 compatible)
    if "ES_HOST" in os.environ:
        try:
            _es = Elasticsearch(
                    ...
                    verify_certs=False,          # OK für HTTP / self-signed
                    request_timeout=60,
                    retry_on_timeout=True,
                    max_retries=5
                )

            # ✅ Elasticsearch-8-safe connectivity check
            info = _es.info()
            logger.info(
    ...
    if no_test and _es is not None:
        _es.indices.delete(index=entity_index_name, ignore=[404])
            ...
            with gzip.open(file_path, 'rb') as file:
                ...
                for line in file.readlines():
                    ...
                    if 'name' in raw_entity \
                        ...
                        if with_language_detection and ld.check_language_is_not_german(raw_entity):
                            ...
                            # Determine duplicates based on entity values without desc
                            entity_wo_desc = entity.copy()
                            if 'desc' in entity_wo_desc:
                                del entity_wo_desc['desc']

                            if 'name' in entity and \
                                (not entity_length_check or len(entity.keys()) > 1):
                                if duplicate_check:
                                    if entity_wo_desc not in found_entities:
                                        found_entities.append(entity_wo_desc)
