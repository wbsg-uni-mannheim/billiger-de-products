...
def load_es_index_configuration(tokenizer):
    ...
    with open('src_blocking/strategy/es_index.yml') as f:
...
def load_ground_truth_tables_for_filtering(schema_org_class):
    ...
    path_to_ground_truth_tables = 'src_blocking/config/ground_truth_tables.json'
...
class RetrievalStrategy:
    ...
    def __init__(self, dataset, name, clusters=False, switched=False):
        ...
        if "ES_HOST" in os.environ:
            try:
                ...
                self.logger.exception("Elasticsearch connection failed")
            ...
            self.processed_records.set_index("id", inplace=True) #Ksenia Added


        self.path_to_table_corpus = 'data/derived/block_benchmark/'
    ...
    def retrieve_evidence(self, query_table, evidence_count, entity_id):
        ...
        """
        logger = logging.getLogger()
        logger.warning('Method in RetrievalStrategy not implemented!')
    ...
    def query_tables_index_by_id(self, ids, index):
        if self._es is None:
            ...
            #Ksenia added for faster indexing
            selected_records = (
                self.processed_records
                .reindex(ids)          # schneller indexed lookup
                .dropna(how="all")     # falls eine ID fehlt
                .to_dict("records")
            )
