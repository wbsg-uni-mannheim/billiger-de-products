...
from src_blocking.strategy.indexing.faiss_collector import determine_path_to_faiss_index
from src_blocking.strategy.indexing.preprocess_records_and_index_es import prepare_processed_action
from src_blocking.strategy.retrieval.encoding.bi_encoder_factory import select_bi_encoder
...
class QueryByNeuralEntity(RetrievalStrategy):
    ...
    def retrieve_evidence(self, query_table, evidence_count, entity_id):
        ...
        if entity_vectors.ndim == 1:
            entity_vectors = entity_vectors.reshape(1, -1)
        ...
        for i in range(0, len(I)):
            ...
            for hit in hits[:evidence_count]:
                ...
                evidence_id += 1
        # TODO
        logger.warning('Elastic search is running? {}'.format(self._es))
        logger.warning('Retrieved {} evidences for query table {}'.format(len(evidences), query_table.identifier))
        return evidences
