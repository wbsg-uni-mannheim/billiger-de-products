...
def preprocess_dataset(arg_tuple):
    ...
    os.makedirs(os.path.dirname(f'{os.environ["DATA_DIR"]}/{handle}/'), exist_ok=True)

    train_df.to_json(f'{os.environ["DATA_DIR"]}/{handle}/{handle}-train.json.gz', compression='gzip', lines=True, orient='records')
    valid['pair_id'].to_csv(f'{os.environ["DATA_DIR"]}/{handle}/{handle}-valid.csv', header=True, index=False)
    gs_df.to_json(f'{os.environ["DATA_DIR"]}/{handle}/{handle}-gs.json.gz', compression='gzip', lines=True, orient='records')
