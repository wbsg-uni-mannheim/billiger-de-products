...
def preprocess_dataset(arg_tuple):
    ...
    left_df = pd.read_csv(f'{os.environ["DATA_DIR"]}/{handle}/tableA.csv', engine='python')
    right_df = pd.read_csv(f'{os.environ["DATA_DIR"]}/{handle}/tableB.csv', engine='python')
    ...
    train = pd.read_csv(f'{os.environ["DATA_DIR"]}/{handle}/train.csv')
    test = pd.read_csv(f'{os.environ["DATA_DIR"]}/{handle}/test.csv')
    valid = pd.read_csv(f'{os.environ["DATA_DIR"]}/{handle}/valid.csv')
    
    full = train._append(valid, ignore_index=True)._append(test, ignore_index=True)
    ...
    train = train._append(valid, ignore_index=True)
    ...
    train_left['product_id'] = train_left['id'].apply(assign_clusterid, args=(product_id_dict, product_id_amount))
    train_right['product_id'] = train_right['id'].apply(assign_clusterid, args=(product_id_dict, product_id_amount))
    gs_left['product_id'] = gs_left['id'].apply(assign_clusterid, args=(product_id_dict, product_id_amount))
    gs_right['product_id'] = gs_right['id'].apply(assign_clusterid, args=(product_id_dict, product_id_amount))
    ...
    entity_set = entity_set._append(right_df[right_df['id'].isin(merged_ids)])
    ...
    entity_set['product_id'] = entity_set['id'].apply(assign_clusterid, args=(product_id_dict, product_id_amount))

    # assign increasing integer label to single nodes
    single_entities = entity_set[entity_set['product_id'] == product_id_amount].copy()
    ...
    entity_set = entity_set._append(single_entities)
    entity_set = entity_set.reset_index(drop=True)
    
    print(f'Amount of entity descriptions: {len(entity_set)}')
    print(f'Amount of clusters: {len(entity_set["product_id"].unique())}')

    os.makedirs(os.path.dirname(f'{os.environ["DATA_DIR"]}/{handle}/contrastive/'), exist_ok=True)

    entity_set.to_pickle(f'{os.environ["DATA_DIR"]}/{handle}/contrastive/{handle}-train.pkl.gz', compression='gzip')
...
if __name__ == '__main__':
    ...
    datasets = [
        ('products80cc20rnd_050un', 'tablea', 'tableb')
    ]
