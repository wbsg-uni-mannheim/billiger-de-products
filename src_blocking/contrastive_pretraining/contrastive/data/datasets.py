...
from src_blocking.contrastive_pretraining.contrastive.data.augmentation import delete_random_tokens
from src_blocking.strategy.entity_serialization import EntitySerializer
...
def serialize_sample_lspc(sample):
    ...
    string = f'{string} [COL] name [VAL] {" ".join(sample[f"name"].split(" ")[:50])}'.strip()
    string = f'{string} [COL] desc [VAL] {" ".join(sample[f"desc"].split(" ")[:100])}'.strip()

    return string

def serialize_sample_wdcproducts(sample, without_special_tokens_and_attribute_names=False):
    entity_serializer = EntitySerializer('products')
    dict_sample = sample.to_dict()
    dict_sample['name'] = dict_sample['name']
    ...
    # string = f'{string} [COL] name [VAL] {" ".join(sample[f"name"].split())}'.strip()
...
#
#         stopwords = ['[COL]', '[VAL]', 'name', 'name', 'desc', 'manufacturer', 'brand', 'specTableContent']
#
...
class ContrastivePretrainDataset(torch.utils.data.Dataset):
    def __init__(self, path, deduction_set, tokenizer='huawei-noah/TinyBERT_General_4L_312D', max_length=128, intermediate_set=None, clean=False, dataset='products80cc20rnd000un', only_interm=False, aug=False):
    ...
    def _prepare_data(self, data):
        ...
        elif self.dataset == 'products80cc20rnd050un':
            data['features'] = data.apply(serialize_sample_wdcproducts, axis=1)

        elif self.dataset == 'products80cc20rnd000un':
            data['features'] = data.apply(serialize_sample_wdcproducts, axis=1)
...
class ContrastivePretrainDatasetSSV(torch.utils.data.Dataset):
    def __init__(self, path, deduction_set, tokenizer='huawei-noah/TinyBERT_General_4L_312D', max_length=128,
                 intermediate_set=None, clean=False, dataset='products80cc20rnd000un', only_interm=False, aug=False):
...
class ContrastivePretrainDatasetDeepmatcher(torch.utils.data.Dataset):
    def __init__(self, path, deduction_set, tokenizer='huawei-noah/TinyBERT_General_4L_312D', max_length=128, intermediate_set=None, clean=False, dataset='products80cc20rnd00un', aug=False, split=True, dataset_type='train+valid', serialization=False):
        ...
        if clean:
            ...
            if dataset == 'products80cc20rnd050un':
            ...
            elif dataset == 'products80cc20rnd000un':
                val = pd.read_csv(f'{os.environ.get("DATA_DIR")}/products80cc20rnd_050un/products80cc20rnd_050un-valid.csv') #Get DATA DIR from env variable
...
class ContrastiveClassificationDataset(torch.utils.data.Dataset):
    def __init__(self, path, dataset_type, size=None, tokenizer='huawei-noah/TinyBERT_General_4L_312D', max_length=128, dataset='products80cc20rnd000un', aug=False):
        ...
        if self.dataset_type != 'test':
            if dataset == 'lspc':
                validation_ids = pd.read_csv(f'{os.environ.get("DATA_DIR")}/raw/wdc-lspc/validation-sets/computers_valid_{size}.csv')
            elif dataset == 'products80cc20rnd050un':
                validation_ids = pd.read_csv(f'{os.environ.get("DATA_DIR")}/interim/wdcproducts80cc20rnd050un/wdcproducts80cc20rnd050un-valid.csv')
            elif dataset == 'products80cc20rnd000un':
                validation_ids = pd.read_csv(f'{os.environ.get("DATA_DIR")}/large/large-valid.csv')
    ...
    def serialize_sample_lspc(self, sample, side):
        ...
        string = f'{string} [COL] name [VAL] {" ".join(sample[f"name_{side}"].split(" ")[:50])}'.strip()
        string = f'{string} [COL] desc [VAL] {" ".join(sample[f"desc_{side}"].split(" ")[:100])}'.strip()
        return string
    ...
    def serialize_sample_wdcproduct(self, sample, side):
        ...
        dict_sample['desc'] = dict_sample['desc_{}'.format(side)]
        ...
        # string = f'{string} [COL] name [VAL] {" ".join(sample[f"name_{side}"].split())}'.strip()
