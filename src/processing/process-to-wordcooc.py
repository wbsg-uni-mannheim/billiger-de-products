...
def process_df_columns_to_wordocc(file, columns_preprocess_wordcooc, feature_combinations):
    ...
    if 'products' in file and 'training' in file:
        ...
        valid_df = pd.read_pickle(valid, compression='gzip')
...
def preprocess_wordcooc(file, columns_to_preprocess, feature_combinations, experiment_name, valid_set=None,
    ...
    out_path = f'data/processed_en/wordcooc/{experiment_name}/'
    ...
    print(test_set, isinstance(test_set, type(None)))
    if not isinstance(test_df, type(None)):
        ...
        print(f"Test set saved to: {out_path + new_test_name + '.pkl.gz'}")


if __name__ == '__main__':

    for file in glob.glob('data/processed_en/training-sets/*'): # Adjusted
        ...
        test ='data/processed_en/gold-standards_adjusted/preprocessed_{}_gs.pkl.gz'.format(test_cat) #Adjusted
