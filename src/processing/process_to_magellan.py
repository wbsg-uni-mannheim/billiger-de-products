...
def preprocess_magellan(file, columns_to_preprocess, experiment_name, validation_set=None):
    ...
    out_path1 = f'data/processed_en/magellan/{experiment_name}/'
    out_path2 = f'data/processed_en/magellan/{experiment_name}/formatted/'
...
if __name__ == '__main__':
    ...
    for file in glob.glob('data/processed_en/training-sets/*'):
        ...
        """if 'products' not in file:
            ...
            valid = valid.replace('processed', 'derived')
    ...
    for file in glob.glob('data/processed_en/gold-standards_adjusted/*'):
        if 'multi' in file or 'products' not in file:
            continue
        if 'products' not in file:
            columns_to_preprocess = ['name', 'desc', 'brand']
