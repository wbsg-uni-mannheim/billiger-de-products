...
def preprocess_dataset():
    os.makedirs('data/processed_en/ditto/data/final_output/', exist_ok=True)
    os.makedirs('data/processed_en/hiergat/data/final_output/', exist_ok=True)

    test_path =  "data/processed_en/gold-standards_adjusted"
    valid_path = "data/processed_en/validation-sets"
    train_path = "data/processed_en/training-sets"
    ...
    for path in glob.glob(os.path.join(test_path, (r'preprocessed_products[!multi]*.pkl.gz'))):
        filename = os.path.basename(path).replace('.pkl.gz', '')
        test_df = preprocess_part(path)
        np.savetxt(f'data/processed_en/ditto/data/final_output/{filename}.txt', test_df.values, fmt = "%s")
        np.savetxt(f'data/processed_en/hiergat/data/final_output/{filename}.txt', test_df.values, fmt = "%s")
    print(f'DONE TEST')

    for path in glob.glob(os.path.join(valid_path, (r'preprocessed_products[!multi]*.pkl.gz'))):
        filename = os.path.basename(path).replace('.pkl.gz', '')
        valid_df = preprocess_part(path)
        np.savetxt(f'data/processed_en/ditto/data/final_output/{filename}.txt', valid_df.values, fmt = "%s")
        np.savetxt(f'data/processed_en/hiergat/data/final_output/{filename}.txt', valid_df.values, fmt = "%s")
    print(f'DONE VALID')

    for path in glob.glob(os.path.join(train_path, (r'preprocessed_products[!multi]*.pkl.gz'))):
        filename = os.path.basename(path).replace('.pkl.gz', '')
        train_df = preprocess_part(path)
        np.savetxt(f'data/processed_en/ditto/data/final_output/{filename}.txt', train_df.values, fmt = "%s")
        np.savetxt(f'data/processed_en/hiergat/data/final_output/{filename}.txt', train_df.values, fmt = "%s")
    print(f'DONE TRAIN')
