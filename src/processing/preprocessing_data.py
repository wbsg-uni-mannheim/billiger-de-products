...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.dirname('data/processed/training-sets/'), exist_ok=True)
    os.makedirs(os.path.dirname('data/processed/gold-standards/'), exist_ok=True)
    os.makedirs(os.path.dirname('data/processed/validation-sets/'), exist_ok=True)

    for file in tqdm(glob.glob('data/derived/training-sets/*')):
        if 'products' in file and 'multi' not in file:
            ...
            df.to_pickle(f'data/processed/training-sets/{file}')
    ...
    for file in glob.glob('data/derived/validation-sets/*'):
        if 'products' in file and 'multi' not in file:
            df = pd.read_json(file, lines=True)
            df['name_left'] = df['name_left'].apply(utils.clean_string_2020)
            df['desc_left'] = df['desc_left'].apply(utils.clean_string_2020)
            ...
            file = file.replace('.json.gz', '.pkl.gz')
            file = f'preprocessed_{file}'
            df.to_pickle(f'data/processed/validation-sets/{file}')
    ...
    for file in glob.glob('data/derived/gold-standards/*'):
        if 'products' in file and 'multi' not in file:
            df = pd.read_json(file, lines=True)
            df['name_left'] = df['name_left'].apply(utils.clean_string_2020)
            df['desc_left'] = df['desc_left'].apply(utils.clean_string_2020)
            ...
            file = file.replace('.json.gz', '.pkl.gz')
            file = f'preprocessed_{file}'
            df.to_pickle(f'data/processed/gold-standards/{file}')
