...
def run_with_tracking(job_name, func, *args, electricity_price_eur_per_kwh=0.30, **kwargs):
    ...
    os.makedirs("data/efficiency_tracker/magellan", exist_ok=True)
    json_path=f"data/efficiency_tracker/magellan/{job_name}.json"
    csv_path = f"data/efficiency_tracker/magellan/{job_name}.csv"
...
def train_single_model(
    ...
    results_out = f"src/models/magellan/model_output_adjusted_ts/predictions/magellan/{experiment_name}/"
    ...
    if write_test_set_for_inspection:
        ...
        out_path = f'data/processed/inspection/magellan/{experiment_name}/'
    ...
    dump(model, f'src/models/magellan/model_output_adjusted_ts/models/{experiment_name}/{report_train_name}_{report_test_name}_{k}_{feature_report}_{run}.joblib')

    with open(f'src/models/magellan/model_output_adjusted_ts/reports/{experiment_name}/{report_train_name}_{report_test_name}.csv', "a") as f:
...
def run_magellan(train_set, valid_set, test_set, feature_combinations, classifiers, experiment_name,
    ...
    os.makedirs(os.path.dirname(f'src/models/magellan/model_output_adjusted_ts/reports/{experiment_name}/'),
                exist_ok=True)

    os.makedirs(os.path.dirname(f'src/models/magellan/model_output_adjusted_ts/models/{experiment_name}/'),
                exist_ok=True)

    with open(f'src/models/magellan/model_output_adjusted_ts/reports/{experiment_name}/{report_train_name}_{report_test_name}.csv',
              "w") as f:
        f.write(
    ...
    for run in range(1, 4):
        for feature_combination in feature_combinations:
            ...
            print("Test:", test_path + '/' + test_set_left)
            ...
            for k, v in classifiers.items():
                ...
                job_name = f"{experiment_name}_{k}_run{run}_testset={test_file}_trainset={train_file}"
...
if __name__ == '__main__':
    ...
    for file in glob.glob('data/processed/magellan/learning-curve/formatted/*'):
        ...
        if 'train_' in file and 'pairs' in file and 'metadata' not in file:
            ...
            test = 'data/processed/magellan/learning-curve_adjusted/formatted/{}_gs_magellan_pairs_formatted.csv'.format(
                test_cat)
