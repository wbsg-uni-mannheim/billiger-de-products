...
def parse_dataset_name(name, pred=False):
    if pred:  
        ...
        """
        "final_large_20cc80rnd000un_lr=5e-06_id=0_batch=16_lm=roberta_english_predictions_050"
        """

        m = re.search(
            r"final_(small|medium|large)_(\d+)cc\d+rnd(\d+)un_lr=([^_]+).*?_id=(\d+)_batch=\d+_lm=([^_]+)_english_predictions_(\d+)",
            name
        )
        if not m:
            m = re.search(
                r"final_(small|medium|large)_(\d+)cc\d+rnd(\d+)un_lr=([^_]+).*?_id=(\d+)_batch=\d+_lm=([^_]+)_english_predictions",
                name
            )
        ...
        """
    "final_large_20cc80rnd000un_seed0_english"
    """

    m = re.search(
        r"final_(small|medium|large)_(\d+)cc\d+rnd(\d+)un_seed(\d+)_english",
        name
    )
    if not m:
        raise ValueError(f"Cannot parse: {name}")
...
def hiergat_results_analysis():
    ...
    cc_records = []
    for jf in glob.glob("data/efficiency_tracker/hiergat_en/*.json"):
    ...
    for tf in glob.glob("src/models/hiergat/output_en/prediction/*.csv", recursive=True):
    ...
    print(perf_df[perf_df["run"] == 2])
    print(carbon_df[carbon_df["run"] == 2])
    ...
    out = Path("src/models/hiergat/analysis/hiergat_experiment_summary_en.xlsx")
...
def categories():
    ...
    predictions_path = "src/models/hiergat/output_en/prediction/*"
    ...
    os.makedirs("src/models/hiergat/analysis/categories", exist_ok=True)
    category_analysis_df.to_csv("src/models/hiergat/analysis/categories/category_analysis_en.csv", index=False)
    ...
    category_summary.to_excel("src/models/hiergat/analysis/categories/category_summary_en.xlsx", index=False)
