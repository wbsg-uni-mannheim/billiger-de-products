...
def parse_dataset_name(name, pred=False):
    if pred:  
        ...
        """
        "final_small_20cc80rnd000un_lr=5e-06_id=1_batch=16_lm=roberta_adjusted_predictions_100"
        """

        m = re.search(
            r"final_(small|medium|large)_(\d+)cc\d+rnd(\d+)un_lr=([^_]+).*?_id=(\d+)_batch=\d+_lm=([^_]+)_adjusted_predictions_(\d+)",
            name
        )
        if not m:
            m = re.search(
                r"final_(small|medium|large)_(\d+)cc\d+rnd(\d+)un_lr=([^_]+).*?_id=(\d+)_batch=\d+_lm=([^_]+)_adjusted_predictions",
                name
            )
        ...
        """
    "final_large_20cc80rnd000un_seed0"
    """

    m = re.search(
        r"final_(small|medium|large)_(\d+)cc\d+rnd(\d+)un_seed(\d+)",
        name
    )
    if not m:
        raise ValueError(f"Cannot parse: {name}")
...
def hiergat_results_analysis():
    ...
    cc_records = []
    for jf in glob.glob("data/efficiency_tracker/hiergat/*.json"):
    ...
    for tf in glob.glob("src/models/hiergat/output/prediction_de/*.csv", recursive=True):
    ...
    merged = perf_df.merge(
        carbon_df,
        on=["Corner Cases", "Train Size", "Test Unseen", "run"],
        how="left"
    )
    ...
    group_cols = ["Corner Cases", "Train Size", "Test Unseen"]
    ...
    out = Path("src/models/hiergat/output/analysis/hiergat_experiment_summary_de.xlsx")
...
def categories():
    ...
    column_path = "data/processed/gold-standards_adjusted"
    predictions_path = "src/models/hiergat/output/prediction_de/*"
    ...
    for file in glob.glob(predictions_path):
        ...
        # final_large_20cc80rnd000un_lr=5e-06_id=0_batch=16_lm=roberta_adjusted_predictions_050
        # final_small_20cc80rnd000un_lr=5e-06_id=1_batch=16_lm=roberta_adjusted_predictions
        ...
        cc_2 = m.group(3)
        run_id = m.group(5)
        gs_name = f"products{cc}cc{cc_2}rnd{test_unseen}un"
    ...
    os.makedirs("src/models/hiergat/output/analysis/categories", exist_ok=True)
    category_analysis_df.to_csv("src/models/hiergat/output/analysis/categories/category_analysis_de.csv", index=False)
    ...
    category_summary.to_excel("src/models/hiergat/output/analysis/categories/category_summary_de.xlsx", index=False)

if __name__ == "__main__":
    hiergat_results_analysis()
    categories()
