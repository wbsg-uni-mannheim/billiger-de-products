...
def build_batch_file(cc, un, batch_path):
    src = f"data/derived/gold-standards_adjusted/products{cc}rnd{un}un_gs.json.gz" #Adjusted Be Aware Here english dataset
    ...
    with gzip.open(src, "rt", encoding="utf-8") as infile, \
        ...
        for i, line in enumerate(infile):
            ...
            prompt = (
                "Du bist ein Experte für Produktabgleich. Deine Aufgabe ist zu entscheiden, ob sich zwei Produktdatensätze auf das EXAKT gleiche Produkt beziehen (gleiche GTIN/SKU).\n"
                "Analysiere die bereitgestellten Datensätze sorgfältig und gib deine Entscheidung strikt als Ja oder Nein zurück.\n\n"
                "KRITISCH: Produktvarianten sind KEINE Übereinstimmungen. Unterschiedliche Größen, Farben, Konfigurationen oder Verpackungsmengen sind UNTERSCHIEDLICHE Produkte mit unterschiedlichen GTINs.\n\n"
                "Richtlinien:\n"
...
def print_f1(cc, un, additional_naming=""):
    df = pd.read_csv(f"src/models/gpt/reports_de/{args.gptmodel}/csv_results/products_{cc}_{un}un_batched_german_{additional_naming}.csv")
    ...
    os.makedirs(f"src/models/gpt/reports_de/{args.gptmodel}/f1", exist_ok=True)
    with open(f"src/models/gpt/reports_de/{args.gptmodel}/f1/f1_score_{cc}_{un}un_german_{additional_naming}.txt", "w", encoding="utf-8") as f:
...
def run_with_tracking(additional_naming=""):
    os.makedirs(f"data/efficiency_tracker/gpt_de/{args.gptmodel}", exist_ok=True)

    csv_file = f"data/efficiency_tracker/gpt_de/{args.gptmodel}/{args.gptmodel}_cc{args.cc}_un{args.un}_batched_german_{additional_naming}.csv"
    job_name = f"{args.gptmodel}_cc{args.cc}_un{args.un}_batched_german_{additional_naming}"
    json_path = f"data/efficiency_tracker/gpt_de/{args.gptmodel}/efficiency_{args.cc}_{args.un}un_batched_german_{additional_naming}.json"
    ...
    os.makedirs(f"src/models/gpt/reports_de/{args.gptmodel}/csv_results", exist_ok=True)
    out_csv = f"src/models/gpt/reports_de/{args.gptmodel}/csv_results/products_{args.cc}_{args.un}un_batched_german_{additional_naming}.csv"
...
# python src/models/gpt/gpt_batch_german_new_prompt.py --cc="20cc80" --un="100" --gptmodel="gpt-5.2"