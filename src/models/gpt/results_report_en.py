...
# === CONFIGURATION PATHS
# =========================================================
additional_info = "hard_prompt"  # e.g., "new_prompt", "old_prompt", "other_experiment_info"
GPT_RESULTS_GLOB = "src/models/gpt/reports_en/gpt-5.2/csv_results/*.csv"
CARBON_LOGS_GLOB = "data/efficiency_tracker/gpt_en/gpt-5.2/efficiency_*.json"

OUTPUT_EXCEL = Path(f"src/models/gpt/analysis/gpt_experiment_summary_en_{additional_info}.xlsx")
...
def parse_gpt_job(job_name):
    ...
    pattern = rf"^gpt-(\d+(?:\.\d+)?)_cc(\d+)cc(\d+)_un(\d+)_batched_english_{re.escape(additional_info)}$"
