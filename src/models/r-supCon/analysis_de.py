...
def parse_dataset_name_rsup_de(name, efficiencytracker):
    if efficiencytracker:
        ...
        corner_cases = m.group(3)
        run_id = int(m.group(6))
        lm_model = m.group(7)
        return {
            "Corner Cases": f"{corner_cases}%",
...
def transformer_results_analysis_de():
    ...
    # ------------------------------------------------------------------
    cc_records = []
    for jf in glob.glob(f"data/efficiency_tracker/r_supCon/*.json"):
    ...
    for folder in glob.glob(f"src/models/r-supCon/reports/contrastive-ft-siamese/*"):
    ...
    out = Path("src/models/r-supCon/reports/analysis/r_supCon_experiment_summary_de.xlsx")
...
def categories():
    ...
    predictions_path = "src/models/r-supCon/reports/predictions/*"
    ...
    os.makedirs("src/models/r-supCon/reports/analysis/categories", exist_ok=True)
    category_analysis_df.to_csv("src/models/r-supCon/reports/analysis/categories/category_analysis_de.csv", index=False)
    ...
    category_summary.to_excel("src/models/r-supCon/reports/analysis/categories/category_summary_de.xlsx", index=False)
