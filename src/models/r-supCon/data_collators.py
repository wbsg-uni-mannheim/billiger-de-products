...
class DataCollatorContrastivePretrain:
    ...
    def __call__(self, input):
        ...
        # inside your data collator __call__
        features_left = [_flatten_text(f) for f in features_left]
        features_right = [_flatten_text(f) for f in features_right]
...
class DataCollatorMatrix:
    ...
    def __call__(self, input):
        ...
        batch['labels'] = pairwise_diff.reshape(len(batch['input_ids']), -1) # type: ignore
