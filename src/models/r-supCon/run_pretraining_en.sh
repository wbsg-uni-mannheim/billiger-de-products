#!/bin/bash
#SBATCH --job-name=rsupcon_pretrain_en
...
for category in products20cc80rnd000un products50cc50rnd000un products80cc20rnd000un; do
  for size in small medium large; do
    python -u src/models/r-supCon/run_pretraining.py \
      ...
      --id_deduction_set "data/solute_en/training-sets/${category}_train_${size}.json.gz" \
      --tokenizer=roberta-base \
      --grad_checkpoint=True \
      --output_dir "results/generated/r-supcon/en/pretrain/${category}-${size}/" \
      --temperature=0.07 \
