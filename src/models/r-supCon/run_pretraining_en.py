...
from modeling import ContrastivePretrainModel
from dataset_en import ContrastivePretrainDataset
from data_collators import DataCollatorContrastivePretrain
...
def main():
    ...
    if training_args.do_train:
        ...
        if data_args.interm_file is not None:
            train_dataset = ContrastivePretrainDataset(train_dataset, tokenizer=model_args.tokenizer, intermediate_set=data_args.interm_file, only_interm=data_args.only_interm, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, only_name=data_args.only_name)
        else:
            train_dataset = ContrastivePretrainDataset(train_dataset, tokenizer=model_args.tokenizer, only_interm=data_args.only_interm, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, only_name=data_args.only_name)
        ...
        if training_args.load_best_model_at_end:
            ...
            test_dataset = ContrastivePretrainDataset(test_dataset, tokenizer=model_args.tokenizer, only_interm=data_args.only_interm, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, only_name=data_args.only_name)

    elif training_args.do_eval:
        if "validation" not in raw_datasets:
            raise ValueError("--do_eval requires a validation dataset")
        validation_dataset = raw_datasets["validation"]
        validation_dataset = ContrastivePretrainDataset(validation_dataset, tokenizer=model_args.tokenizer, only_interm=data_args.only_interm, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, only_name=data_args.only_name)
    ...
    trainer = Trainer(
        ...
        compute_metrics=None #changed from compute_metrics_bce to None
    )
...
def run_with_tracking(job_name, func, *args, gpt_usage=False, electricity_price_eur_per_kwh=0.30, **kwargs):
    ...
    json_path = f"data/efficiency_tracker/r_supcon/pretraining_{job_name}.json"
    csv_path = f"data/efficiency_tracker/r_supcon/pretraining_{job_name}.csv"
