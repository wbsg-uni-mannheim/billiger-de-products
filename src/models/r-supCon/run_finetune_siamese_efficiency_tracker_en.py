...
from dataset_en import ContrastiveClassificationDataset
...
def run_with_tracking(job_name, func, *args, electricity_price_eur_per_kwh=0.30, **kwargs):
    ...
    os.makedirs("data/efficiency_tracker/r_supCon_en", exist_ok=True)
    csv_path = f"data/efficiency_tracker/r_supCon_en/{job_name}.csv"
    json_path = f"data/efficiency_tracker/r_supCon_en/{job_name}.json"
...
def predict_dataset(dataset_path, dataset_label, model_args, data_args, output_dir, unseen, run):
    ...
    pred_out = os.path.join(
        "src/models/r-supCon/reports_en/predictions",
