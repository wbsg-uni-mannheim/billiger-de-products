...
for CATEGORY in "${DATASETS[@]}"; do
	for SIZE in "${SIZES[@]}"; do
		python -u src/models/r-supCon/run_finetune_siamese_efficiency_tracker_en.py \
			--model_pretrained_checkpoint src/models/r-supCon/reports_en/contrastive/$CATEGORY-$SIZE-$AUG$BATCH-$LR-$TEMP-${MODEL##*/} \
			--do_train \
			--frozen=$FROZEN \
			...
			--output_dir src/models/r-supCon/reports_en_/contrastive-ft-siamese/$CATEGORY-$SIZE-$AUG$BATCH-$PREAUG$LR-$TEMP-$FROZEN-${MODEL##*/}_adjusted/ \
