    ...
    def __init__(self, len_tokenizer, checkpoint_path, model='huawei-noah/TinyBERT_General_4L_312D', pool=True, frozen=True):
        ...
        if self.checkpoint_path:
            if os.path.isdir(self.checkpoint_path):
                self.checkpoint_path = os.path.join(self.checkpoint_path, "model.safetensors")
            if not self.checkpoint_path.endswith(".safetensors"):
                raise ValueError(f"Expected a .safetensors file, got: {self.checkpoint_path}")
...
class ContrastiveClassifierModel(nn.Module):
    ...
    def __init__(self, len_tokenizer, checkpoint_path, model='huawei-noah/TinyBERT_General_4L_312D', pool=True, comb_fct='concat-abs-diff-mult', frozen=True, pos_neg=False, proj=16):
        ...
        if self.checkpoint_path:
            ...
            # build a corrected encoder state dict
            encoder_state = {}
            for k, v in checkpoint.items():
                ...
                # fix mismatch
                new_k = new_k.replace("transformer.layer", "transformer.encoder.layer")
    ...
    def from_pretrained(
        ...
        *,
        ...
        pool=True,
        comb_fct='concat-abs-diff-mult',
        frozen=True,
        pos_neg=False,
        proj=16,
    ):
        model = cls(
            len_tokenizer=len_tokenizer,
