...
for CATEGORY in "${DATASETS[@]}"; do
	for SIZE in "${SIZES[@]}"; do
		python -u src/models/r-supCon/run_pretraining.py \
			...
			--id_deduction_set data/derived/training-sets/${CATEGORY}_train_$SIZE.json.gz \
			--tokenizer=$MODEL \
			--grad_checkpoint=$CHECKPOINT \
			--output_dir src/models/r-supCon/reports/contrastive/$CATEGORY-$SIZE-$AUG$BATCH-$LR-$TEMP-${MODEL##*/}/ \
			--temperature=$TEMP \
