...
class DittoDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.da is not None:
            combined = self.augmenter.augment_sent(left + ' [SEP] ' + right, self.da) #type: ignore
            if random.random() < 0.01:  # nicht zu viel loggen
                print("[AUG DEBUG]", self.da)
                print("ORIG:", left[:120], "|", right[:120])
                print("AUG :", combined[:240])
