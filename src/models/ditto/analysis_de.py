...
def parse_dataset_name(name, pred=False):
    if pred:  
        ...
        """
        "final_large_20cc80rnd000un_lm=roberta_da=del_dk=None_su=False_size=None_id=0_adjusted_testset_predictions_050"
        """

        m = re.search(
            r"final_(small|medium|large)_(\d+)cc\d+rnd(\d+)un_lm=([^_]+).*?_id=(\d+)_adjusted_testset_predictions_(\d+)",
            name
        )
        if not m:
            m = re.search(
                r"final_(small|medium|large)_(\d+)cc\d+rnd(\d+)un_lm=([^_]+).*?_id=(\d+)_adjusted_testset_predictions",
                name
            )
        ...
        """
    "final_large_20cc80rnd000un_lm=roberta_da=del_dk=None_su=False_size=None_id=0_adjusted_testset"
    "final_large_20cc80rnd000un_lm=roberta_da=del_dk=None_su=False_size=None_id=0_adjusted_testset.txt"
    """

    m = re.search(
        r"final_(small|medium|large)_(\d+)cc\d+rnd(\d+)un_lm=([^_]+).*?_id=(\d+)_adjusted_testset",
        name
    )
    if not m:
        raise ValueError(f"Cannot parse: {name}")
...
def ditto_results_analysis():
    ...
    for tf in glob.glob("src/models/ditto/output/prediction_de/*.csv", recursive=True):
    ...
    out = Path("src/models/ditto/output/analysis/ditto_experiment_summary_de.xlsx")
...
def categories():
    ...
    predictions_path = "src/models/ditto/output/prediction_de/*"
    ...
    os.makedirs("src/models/ditto/output/analysis/categories", exist_ok=True)
    category_analysis_df.to_csv("src/models/ditto/output/analysis/categories/category_analysis_de.csv", index=False)
    ...
    category_summary.to_excel("src/models/ditto/output/analysis/categories/category_summary_de.xlsx", index=False)
