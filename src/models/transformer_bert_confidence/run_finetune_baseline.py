...
def run_with_tracking(job_name, func, *args, electricity_price_eur_per_kwh=0.30, **kwargs):
    ...
    os.makedirs("data/efficiency_tracker/roberta", exist_ok=True)
    csv_path = f"data/efficiency_tracker/roberta/{job_name}.csv"
    json_path = f"data/efficiency_tracker/roberta/{job_name}.json"
    ...
    for run in range(3):
        ...
        if training_args.do_predict or training_args.do_train:
            ...
            predict_results = run_with_tracking(
                job_name=f"roberta_predict_000un_{data_args.train_size}_{test_name}_id={run}",
            ...
            if 'products' in raw_datasets["train"]:
                predict_results = run_with_tracking(
                    job_name=f"roberta_predict_un050_{data_args.train_size}_{test_name}_id={run}",
                    func=trainer.predict,
                ...
                predict_results = run_with_tracking(
                    job_name=f"roberta_predict_un100_{data_args.train_size}_{test_name}_id={run}",
                    func=trainer.predict,
