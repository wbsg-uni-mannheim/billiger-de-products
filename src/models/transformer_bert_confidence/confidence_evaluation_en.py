...
if __name__ == "__main__":
    ...
    results_root = "src/models/transformer_bert_confidence/reports_calibration_en"
    all_rows = []
    ...
    for size in ["small", "medium", "large"]:
        for corner_cases in ["20cc80", "50cc50", "80cc20"]:
            ...
            output_dir = (
                f"src/models/transformer_bert_confidence/reports_en/baseline/"
                f"{base_key}-{size}-all1024-5e-05-roberta-base"
            )

            df0 = pd.read_pickle(
                f"data/processed_en/gold-standards_adjusted/preprocessed_{base_key}_gs.pkl.gz"
            )
            df50 = pd.read_pickle(
                f"data/processed_en/gold-standards_adjusted/preprocessed_{base_key.replace('000un','050un')}_gs.pkl.gz"
            )
            df100 = pd.read_pickle(
                f"data/processed_en/gold-standards_adjusted/preprocessed_{base_key.replace('000un','100un')}_gs.pkl.gz"
            )
