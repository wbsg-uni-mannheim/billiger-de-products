...
def parse_dataset_name_rsup_de(name, efficiencytracker):
...
def transformer_results_analysis_de():
    all_merged = []
    ...
    # ------------------------------------------------------------------
    cc_records = []
    for jf in glob.glob(f"data/efficiency_tracker/roberta/*.json"):
        ...
        for i,rec in enumerate(data):
            ...
            job = rec["job_name"]
            meta = parse_dataset_name_rsup_de(job, True)
            cc_records.append({
                **meta,
                "Corner Cases": f"{cc}%",
    ...
    for folder in glob.glob(f"src/models/transformer_bert_confidence/reports/baseline/*"):
        if folder.__contains__("roberta-base"):
            for id_folder in glob.glob(f"{folder}/*"):
                name = Path(folder).name
                meta = parse_dataset_name_rsup_de(name, False)
    ...
    out = Path("src/models/transformer_bert_confidence/reports/analysis/roberta_experiment_summary_de.xlsx")
...
def categories():
    ...
    predictions_path = "src/models/transformer_bert_confidence/reports/predictions/*"
    ...
    os.makedirs("src/models/transformer_bert_confidence/reports/analysis/categories", exist_ok=True)
    category_analysis_df.to_csv("src/models/transformer_bert_confidence/reports/analysis/categories/category_analysis.csv", index=False)
    ...
    category_summary.to_excel("src/models/transformer_bert_confidence/reports/analysis/categories/category_summary.xlsx", index=False)



if __name__ == "__main__":
    #transformer_results_analysis_de()
    categories()
