...
def run_with_tracking(job_name, func, *args, electricity_price_eur_per_kwh=0.30, **kwargs):
    ...
    os.makedirs("data/efficiency_tracker/wordcooc_en", exist_ok=True)
    json_path = f"data/efficiency_tracker/wordcooc_en/{job_name}.json"
    csv_path = f"data/efficiency_tracker/wordcooc_en/{job_name}.csv"
...
def train_single_model_wordcooc(
    ...
    results_out = f"src/models/wordcooc/model_output_en/predictions/wordcooc/{experiment_name}/"
    ...
    if write_test_set_for_inspection:
        out_path = f'data/processed_en/inspection/wordcooc/{experiment_name}/'
    ...
    # Save model
    dump(model,
         f'src/models/wordcooc/model_output_en/models/wordcooc/{experiment_name}/'
         f'{report_train_name}_{report_test_name}_{k}_{feature_combination}_{run}.joblib')

    # Append report line (same format as before)
    with open(
        f'src/models/wordcooc/model_output_en/reports/wordcooc/{experiment_name}/'
...
def run_wordcooc(train_set, valid_set, test_set, feature_combinations, classifiers, experiment_name,
    ...
    os.makedirs(os.path.dirname(f'src/models/wordcooc/model_output_en/reports/wordcooc/{experiment_name}/'),
                exist_ok=True)

    os.makedirs(os.path.dirname(f'src/models/wordcooc/model_output_en/models/wordcooc/{experiment_name}/'),
                exist_ok=True)


    with open(f'src/models/wordcooc/model_output_en/reports/wordcooc/{experiment_name}/{report_train_name}_{report_test_name}.csv',
              "w") as f:
        f.write(
    ...
    for run in range(1, 4):
        for feature_combination in feature_combinations:
            ...
            for k, v in classifiers.items():
                job_name = (
                    f"{experiment_name}_wordcooc_{k}"
...
if __name__ == '__main__':
    ...
    for file in glob.glob('data/processed_en/wordcooc/learning-curve/*'):
        ...
        if 'train_' in file and '_gs' not in file:
            ...
            test = 'data/processed_en/wordcooc/learning-curve/{}'.format(test)
