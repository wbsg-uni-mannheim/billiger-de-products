        ...
        for feature_combination in feature_combinations:
            ...
            for k, v in classifiers.items():
                ...
                if k == 'LogisticRegression' or k == 'LinearSVC':
                    most_important_features = model.best_estimator_.coef_ # type: ignore 
                ...
                if k == 'RandomForest' or k == 'DecisionTree':
                    most_important_features = model.best_estimator_.feature_importances_# type: ignore 
                ...
                if write_test_set_for_inspection:
                    ...
                    if k == 'LinearSVC':
                        proba_gs = model.decision_function(feats_gs).tolist()# type: ignore 
                    else:
                        proba_gs = model.predict_proba(feats_gs).tolist()# type: ignore 
                ...
                with open(f'model_output/reports/wordcooc_adjusted/{experiment_name}/{report_train_name}_{report_test_name}.csv', "a") as f:
                    f.write(feature_combination + '#####' + k + '#####' + str(
                        ...
                        scores['std_test_score']) + '#####' + str(gs_report['1']['precision']) + '#####' + str(# type: ignore 
                        gs_report['1']['recall']) + '#####' + str(gs_report['1']['f1-score'])# type: ignore 
                            + '#####' + str(parameters) + '#####' + str(train_time) + '#####' + str(pred_time)# type: ignore 
                            + '#####' + str(word_importance[
