...
def wordcooc_results_analysis():
    ...
    cc_records = []
    for jf in glob.glob("data/efficiency_tracker/wordcooc/*.json"):
        ...
        for rec in data:
            ...
            if "learning-curve" in job:
                # Skip old logs that don't have the full info in the job name
                continue
            # Example job:
            # adjusted_wordcooc_DecisionTree_run1_feature=brand+name+price+desc_train=..._test=...
            m = re.search(
                r"adjusted_wordcooc_(.*?)_run(\d+)_feature=(.*?)_train=(.*?)_test=(.*)",
                job
            )
    ...
    for cf in glob.glob("src/models/wordcooc/model_output_adjusted_ts/reports/wordcooc_adjusted/learning-curve_adjusted/*.csv", recursive=True):
    ...
    out = Path("src/models/wordcooc/model_output_adjusted_ts/analysis/wordcooc_experiment_summary_de.xlsx")
...
def file_name_transformation(file):
    #src/models/wordcooc/model_output_adjusted_ts/predictions/wordcooc/learning-curve_adjusted/preprocessed_products50cc50rnd000un_train_small_wordcooc_preprocessed_products50cc50rnd000un_train_small_wordcooc_preprocessed_products50cc50rnd100un_gs_LogisticRegression_2.csv
...
def analyse_confidence_curves(prediction_glob_pattern):
    ...
    best_f1_final = recompute_and_save(
        ...
        "src/models/wordcooc/model_output_adjusted_ts/analysis/best_thresholds_f1.csv"
    )

    best_total_final = recompute_and_save(
        best_total,
        prediction_glob_pattern,
        "src/models/wordcooc/model_output_adjusted_ts/analysis/best_thresholds_total_f1_ge_0_9.csv"
    )
...
def categories():
    ...
    predictions_path = "src/models/wordcooc/model_output_adjusted_ts/predictions/wordcooclearning-curve_adjusted/*.csv"
    ...
    os.makedirs("src/models/wordcooc/model_output_adjusted_ts/analysis/categories", exist_ok=True)
    category_analysis_df.to_csv("src/models/wordcooc/model_output_adjusted_ts/analysis/categories/category_analysis_de.csv", index=False)
    ...
    category_summary.to_excel("src/models/wordcooc/model_output_adjusted_ts/analysis/categories/category_summary_de.xlsx", index=False)
        
def categories_of_best_models():
    category_path= "src/models/wordcooc/model_output_adjusted_ts/analysis/categories/category_summary_de.xlsx"
    model_results_path = "src/models/wordcooc/model_output_adjusted_ts/analysis/wordcooc_experiment_summary_de.xlsx"
    ...
    category_analysis.to_excel("src/models/wordcooc/model_output_adjusted_ts/analysis/categories/category_analysis_best_models_de.xlsx", index=False)

if __name__ == "__main__":
    #wordcooc_results_analysis()
    #analyse_confidence_curves(
    #    "src/models/wordcooc/model_output_adjusted_ts/predictions/wordcooclearning-curve_adjusted/*.csv"
    #)
    categories()
