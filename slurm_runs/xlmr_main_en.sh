#!/bin/bash
#SBATCH --job-name=xlmr_en
#SBATCH --cpus-per-task=10
#SBATCH --mem=70G
#SBATCH --time=48:00:00
#SBATCH --gres=gpu:1
#SBATCH --partition=gpu-vram-48gb
#SBATCH --output=slurm_runs/logs/xlmr_en_%j.out
#SBATCH --error=slurm_runs/logs/xlmr_en_%j.err

# XLM-R baseline on the recommended configuration (80 % corner cases), English version.
...
for size in $SIZES; do
  out="${OUT_ROOT:-results/generated/xlmr}/en/${CATEGORY}-${size}/"
  python -u -m src.cross_language.provenance \
    --output-dir "$out" \
    --model xlmr \
    --backbone "$BACKBONE" \
    --validation-file "data/processed_en/validation-sets/preprocessed_${CATEGORY}_valid_${size}.pkl.gz" \
    --train-file "data/processed_en/training-sets/preprocessed_${CATEGORY}_train_${size}.pkl.gz" \
  ...
  python -u src/models/transformer_bert_confidence/run_finetune_baseline_en.py \
    ...
    --train_size="$size" \
    --validation_file "data/processed_en/validation-sets/preprocessed_${CATEGORY}_valid_${size}.pkl.gz" \
